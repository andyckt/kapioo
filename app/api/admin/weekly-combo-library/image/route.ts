...
export async function POST(request: NextRequest) {
  try {
    ...
    const { url, key } = await uploadMenuImageToS3({
      file,
      prefix: "weekly-combo-library-images",
      identifier: formData.get("weeklyComboLibraryId") as string | null,
    })
    ...
    return NextResponse.json({
      ...
      url,
      key,
      message: "Weekly combo library image uploaded successfully",
    })
  } catch (error: unknown) {
    console.error("Error uploading weekly combo library image to S3:", error)
    return errorJson("Failed to upload weekly combo library image", 500)
  }
}
