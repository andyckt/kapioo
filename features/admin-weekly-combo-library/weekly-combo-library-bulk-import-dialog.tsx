...
const TEMPLATE_HEADERS = ["name", "nameEn", "internalName", "dishes", "calories", "tags", "allergens", "dietaryTags", "description", "notesForAdmin"]

function downloadTemplate() {
  const csv = `${TEMPLATE_HEADERS.join(",")}\n周餐A,,周餐A,菜1;菜2;菜3,650,高蛋白,soy;sesame,,描述,`
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" })
  const url = URL.createObjectURL(blob)
  const link = document.createElement("a")
  link.href = url
  link.download = "weekly-combo-library-template.csv"
  link.click()
  URL.revokeObjectURL(url)
}

export function WeeklyComboLibraryBulkImportDialog({ open, onOpenChange, onImported }: Props) {
  ...
  const preview = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/weekly-combo-library/bulk-import/preview", { method: "POST", body: formData })
  ...
  const commit = async () => {
    ...
    try {
      const response = await fetch("/api/admin/weekly-combo-library/bulk-import/commit", {
        method: "POST",
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
        <DialogHeader><DialogTitle>Bulk Import Weekly Combo Library</DialogTitle></DialogHeader>
