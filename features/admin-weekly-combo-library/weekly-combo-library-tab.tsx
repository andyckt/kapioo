  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div><h1 className="text-2xl font-bold">Weekly Combo Library</h1><p className="text-sm text-muted-foreground">Reusable Weekly Delivery meal options with a single dishes list.</p></div>
      ...
      <Card><CardContent className="grid gap-3 p-4 md:grid-cols-3">
        <Input value={filters.q ?? ""} onChange={(event) => setFilters({ q: event.target.value })} placeholder="Search Weekly combos..." className="md:col-span-2" />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {items.map((item) => (
          <Card key={item.weeklyComboLibraryId}>
            <CardHeader className="pb-3"><div className="flex gap-4">{item.imageUrl ? <img src={item.imageUrl} alt={`${item.name} preview`} className="h-24 w-32 rounded-lg object-cover" /> : <div className="flex h-24 w-32 items-center justify-center rounded-lg bg-muted text-xs">No image</div>}<div className="min-w-0 flex-1"><CardTitle className="text-lg">{item.name}</CardTitle><p className="text-xs text-muted-foreground">{item.weeklyComboLibraryId}</p><div className="mt-2 flex flex-wrap gap-1"><Badge>{item.status}</Badge>{item.calories ? <Badge variant="outline">{item.calories} kcal</Badge> : null}</div></div></div></CardHeader>
            <CardContent className="space-y-3"><p className="text-sm"><span className="font-medium">Dishes:</span> {item.dishes.slice(0, 6).join(" / ") || "-"}</p><div className="flex flex-wrap gap-1">{item.tags.slice(0, 5).map((tag) => <Badge key={tag} variant="secondary">{tag}</Badge>)}</div><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => void editItem(item)}><Pencil className="mr-2 h-4 w-4" />Edit</Button><Button type="button" size="sm" variant="outline" onClick={() => void duplicateItem(item)}><Copy className="mr-2 h-4 w-4" />Duplicate</Button><Button type="button" size="sm" variant="outline" onClick={() => void setStatus(item, item.status === "archived" ? "active" : "archived")}>{item.status === "archived" ? "Restore" : "Archive"}</Button></div></CardContent>
          </Card>
        ))}
      </div>
