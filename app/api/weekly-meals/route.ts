...
import { assignWeeklyMealBodySchema, weeklyMealsWeekYearQuerySchema } from '@/lib/contracts/weekly-meals-api';
...
export async function GET(request: Request) {
  try {
    ...
    console.log(
      '[API] All weekly meals in DB (including inactive):',
    ...
    if (Object.keys(formattedMeals).length === 0) {
      ...
      const inactiveDays = allWeeklyMeals.filter((meal) => meal.active === false).map((meal) => meal.day);
      ...
      defaultMeals.forEach((meal: any) => {
        ...
        if (meal.day && !inactiveDays.includes(meal.day)) {
    ...
    const res = successJson(formattedMeals);
    Object.entries(noStoreHeaders).forEach(([k, v]) => res.headers.set(k, v));
    return res;
  } catch (error: unknown) {
    return handleRouteError(error, 'GET /api/weekly-meals');
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await parseJsonBody(request, assignWeeklyMealBodySchema);
    if (error) {
      return error;
    }
    ...
    return successJson(weeklyMeal, { status: 201 });
  } catch (error: unknown) {
    return handleRouteError(error, 'POST /api/weekly-meals');
  }
}
